//  LoopingVideoPlayerController.swift
//  WeatherCast
//
...
final class LoopingVideoPlayerController {
    private static let transitionDuration = 0.25

    private var activeSlot = PingPongPlayerSlot()
    private var standbySlot = PingPongPlayerSlot()
    ...
    func configure(resourceName: String) {
        ...
        bringStandbyLayerToFront()
        standbySlot.prepare(
            resourceName: resourceName,
            url: url
        ) { [weak self] readyResourceName in
            self?.showStandbyVideo(resourceName: readyResourceName)
        }
    }
    ...
    private func bringStandbyLayerToFront() {
        guard let containerLayer else { return }

        standbySlot.playerLayer.removeFromSuperlayer()
        containerLayer.addSublayer(standbySlot.playerLayer)
        standbySlot.playerLayer.frame = bounds
    }
...
private final class PingPongPlayerSlot {
